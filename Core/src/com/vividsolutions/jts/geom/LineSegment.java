...
public class LineSegment implements Comparable<Object> {
	 ...
	 * concept of orientation is specified as follows: Given two line segments A
	 * and L, <ul <li>A is to the left of a segment L if A lies wholly in the
	 * closed half-plane lying to the left of L <li>A is to the right of a
	 * segment L if A lies wholly in the closed half-plane lying to the right of
	 * L <li>otherwise, A has indeterminate orientation relative to L. This
	 * happens if A is collinear with L or if A crosses the line determined by
	 * L. </ul>
	 * 
	 * @param seg
	 ...
	 * @return 0 if <code>seg</code> has indeterminate orientation relative to
	 *         this segment
	 */
	 ...
	 * {@link CGAlgorithms#computeOrientation}.
	 * 
	 * @param seg
	 *            the LineSegment to compare
	 * 
	...
	/**
	 * Puts the line segment into a normalized form. This is useful for using
	 * line segments in maps and indexes when topological equality rather than
	 * exact equality is desired. A segment in normalized form has the first
	 ...
	 * 
	 * @return the perpendicular distance between the defined line and the given
	 *         point
	 */
	 ...
	 * point of the segment; a fraction of <code>1.0</code> returns the end
	 * point of the segment. If the fraction is < 0.0 or > 1.0 the point
	 * returned will lie before the start or beyond the end of the segment.
	 * 
	...
	/**
	 * Computes the {@link Coordinate} that lies a given fraction along the line
	 * defined by this segment and offset from the segment by a given distance.
	 * A fraction of <code>0.0</code> offsets from the start point of the
	 * segment; a fraction of <code>1.0</code> offsets from the end point of the
	 * segment. The computed point is offset to the left of the line if the
	 * offset distance is positive, to the right if negative.
	 * 
	 * @param segmentLengthFraction
	 *            the fraction of the segment length along the line
	 * @param offsetDistance
	 *            the distance the point is offset from the segment (positive is
	...
	/**
	 * Computes the Projection Factor for the projection of the point p onto
	 * this LineSegment. The Projection Factor is the constant r by which the
	 * vector for this segment must be multiplied to equal the vector for the
	 * projection of <tt>p<//t> on the line
	 ...
	 * projection of a point occurs along this line segment. If the point is
	 * beyond either ends of the line segment, the closest fractional value (
	 ...
	 * segment.
	 * <p>
	 * Note that the projected point may lie outside the line segment. If this
	 * is the case, the projection factor will lie outside the range [0.0, 1.0].
	 */
	 ...
	 * line segment. The returned line segment will be a subset of the target
	 * line line segment. This subset may be null, if the segments are oriented
	 * in such a way that there is no projection.
	 * <p>
	 * Note that the returned line may have zero length (i.e. the same
	 * endpoints). This can happen for instance if the lines are perpendicular
	 * to one another.
	 * 
	 * @param seg
	 ...
	 * @return a Coordinate which is the closest point on the line segment to
	 *         the point p
	 */
	 ...
	 * There may be 0, 1 or many intersection points between two segments. If
	 * there are 0, null is returned. If there is 1 or more, a single one is
	 * returned (chosen at the discretion of the algorithm). If more information
	 * is required about the details of the intersection, the
	...
	/**
	 * Computes the intersection point of the lines defined by two segments, if
	 * there is one. There may be 0, 1 or an infinite number of intersection
	 * points between two lines. If there is a unique intersection point, it is
	 * returned. Otherwise, <tt>null</tt> is returned. If more information is
	 * required about the details of the intersection, the
	 * {@link RobustLineIntersector} class should be used.
	 * 
	 * @param line
	 *            a line segment defining a straight line
	 * @return an intersection point, or <code>null</code> if there is none or
	 *         an infinite number
	 * 
	 * @see RobustLineIntersector
	 */
	...
	/**
	 * Compares this object with the specified object for order. Uses the
